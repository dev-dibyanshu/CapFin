...
public class ApplicationsController : ControllerBase
{
    ...
    [Authorize(Roles = RoleNames.Applicant)]
    public async Task<IActionResult> UpdatePersonalDetails(Guid id, [FromBody] PersonalDetailsRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var application = await _loanApplicationService.UpdatePersonalDetailsAsync(id, GetUserId(), request, cancellationToken);
            return Ok(application);
        }
        catch (KeyNotFoundException exception)
        {
    ...
    [Authorize(Roles = RoleNames.Applicant)]
    public async Task<IActionResult> UpdateEmploymentDetails(Guid id, [FromBody] EmploymentDetailsRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var application = await _loanApplicationService.UpdateEmploymentDetailsAsync(id, GetUserId(), request, cancellationToken);
            return Ok(application);
        }
        catch (KeyNotFoundException exception)
        {
    ...
    [HttpPut("{id:guid}/loan-details")]
    [Authorize(Roles = RoleNames.Applicant)]
    public async Task<IActionResult> UpdateLoanDetails(Guid id, [FromBody] LoanDetailsRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var application = await _loanApplicationService.UpdateLoanDetailsAsync(id, GetUserId(), request, cancellationToken);
            return Ok(application);
        }
        catch (KeyNotFoundException exception)
        {
